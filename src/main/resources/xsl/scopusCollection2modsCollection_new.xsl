...
<xsl:stylesheet version="1.0"
	...
	<xsl:param name="askScopus.ScopusAPI" />
  
  <xsl:output method="xml" encoding="UTF-8" indent="yes" xalan:indent-amount="2" />
  
  <xsl:include href="abstract_templates.xsl"/>
  
  <xsl:template match="/">
    <mods:modsCollection xsi:schemaLocation="http://www.loc.gov/mods/v3 http://www.loc.gov/standards/mods/v3/mods-3-4.xsd">
      <xsl:apply-templates select="documents/abstract-document"/>
    </mods:modsCollection>
  </xsl:template>
  
  
  <xsl:template match="/affiliation-retrieval-response/documents/abstract-document">
    <xsl:apply-templates select="link[@rel='self']" />
  </xsl:template>
  
  <xsl:template match="link[@rel='self']">
    <xsl:apply-templates select="document(concat(@href,'?apiKey=',$scopus.api.key))/scopus:abstracts-retrieval-response"/>
  </xsl:template>
