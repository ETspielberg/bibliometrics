...
public class IEEEConnector {
    ...
    static {
        ...
        API_URL = config.getString("MCR.api.IEEEURI");
    }

    public IEEEConnector() {
    }

    public MCRContent getPublicationByDOI(String doi) throws HttpException, IOException {
        //build API URL
        String queryURL = API_URL + "doi=" + doi;
        return getResponse(queryURL);
    }
    
    public MCRContent getPublicationByAuthor(String author) throws HttpException, IOException {
        //build API URL
        String queryURL = API_URL + "au=" + author.replace(" ", "+");
        return getResponse(queryURL);
    }
